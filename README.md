...
- **News-spike summaries (PRD-021/022/026)** — when a query is currently trending AND the article has a "recent dated event" paragraph (or, fallback, Google News headlines from the past 7 days), the knowledge panel shows a news-flavoured summary instead. The article-specificity gate doubles as the trending rail's quality filter. PRD-026 split the rail sources into spike-pipeline + Google News top stories + Wikipedia "In the news" portal, with source-weighted sorting (Google → spike → ITN). The gate is freshness-aware: a Wikipedia paragraph whose newest date is ≥2 days old gets swapped for the Google News headline synthesis when the headlines are meaningfully newer (`STALE_EVENT_DAYS` / `SWAP_FRESHER_BY_DAYS` in fetch_trending.py) — summary freshness is bounded by the news, not by Wikipedia editor activity.
...
- **2026-06-09 — week-stale "In the news" panel on a spiking query (Tom Steyer)**. Chip on the rail implied "news now"; the panel summary referenced June 2 events. Root cause: the specificity gate always preferred a qualifying Wikipedia paragraph (≤30d old) over Google News, so summary freshness was bounded by Wikipedia editor activity — when editors stop updating, the paragraph hash freezes and the producer's refresh machinery regenerates the same stale summary. Fix: freshness-aware swap in the gate (stale wiki paragraph + meaningfully-newer headlines → headline synthesis, which also changes the hash and triggers regeneration); `STALE_NEWS_DAYS_SERVE` tightened 30 → 7 to match the quality filter's headline window; producer skips jobs whose event_date is already >7d (`NEWS_MAX_EVENT_AGE_DAYS`).
...
zettair-trending.timer (every 1 h)
  → tools/fetch_trending.py
    ...
    • PRD-021/022: specificity gate fetches each candidate's Wikipedia article
      ...
      RSS headlines for the query. A wiki paragraph ≥2d old is swapped for the
      headline synthesis when Google News is ≥1d newer (stale-source fix,
      2026-06-10 — the Tom Steyer case)
